...
export default function DfsSurvivorWeekComponent({
  ...
  week.entries.forEach(entry => {
    ...
    if (
      ...
      entry.player.currentNFLTeam?.sleeperId
    ) {
  ...
  const isSaveDisabled = useCallback(() => {
    ...
    if (
      weekGameTiming &&
      currentTime &&
      currentTime >= weekGameTiming.lastGameStartTime
    ) {
      return true;
    }
  ...
  const isPlayerInputDisabled = useCallback(
    (position: string, playerId?: string) => {
  ...
  const checkForIntraWeekDuplicates = useCallback(() => {
    ...
    const playerCounts = new Map<
      string,
      { name: string; positions: Set<string> }
    >();
    ...
    for (const [, info] of playerCounts) {
      ...
      if (info.positions.size > 1) {
        ...
        const error = `Cannot select ${info.name} multiple times in week ${
          week.week
        } (${positionsArray.join(', ')})`;
  ...
  const getPositionPlayersArray = useCallback(
            ...
            <form method='post' id={formId} onSubmit={handleSubmit}>
              ...
              {positions.map(position => {
                ...
                return (
                  <div
                    key={position}
                    className='header-row flex justify-between items-center mb-2'
                      ...
                      <div className='flex-1'>
                        <SearchSelect
                          ...
                          onOptionSelect={playerName =>
                            handlePlayerSelect(position, playerName)
                          }
                          onOptionSelectedChange={value =>
                            handlePlayerSelect(position, value)
                          }
