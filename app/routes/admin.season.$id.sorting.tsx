...
function trySlotCombination(
  ...
  for (const player of shuffledPlayers.filter(p => !p.hasPreferences)) {
    ...
    const availableInCombo = slotCombo.filter(slot =>
      player.availableSlots.includes(slot.id),
    );
    const shuffledAvailable = [...availableInCombo].sort(
      () => Math.random() - 0.5,
    );
...
function calculateCombinationScore(
  ...
  // TODO: Check to make sure this is actually unused
  ...
  if (result.slotAvailabilityAnalysis.length > 0) {
    const maxAvailableCount = Math.max(
      ...result.slotAvailabilityAnalysis.map(
        analysis => analysis.availablePlayerCount,
      ),
    );
...
export default function LeagueSorting() {
  ...
  const preservedLeagueAssignments =
    actionData && 'leagueAssignments' in actionData
      ? actionData.leagueAssignments
      : {};
  const [leagueAssignments, setLeagueAssignments] = useState<
    Record<number, string>
  >(preservedLeagueAssignments || {});

  // Sync league assignments state with action data when it changes
  useEffect(() => {
    if (
      actionData &&
      'leagueAssignments' in actionData &&
      actionData.leagueAssignments
    ) {
  ...
  const handleInvertSelection = () => {
    ...
    const newSelectedPlayers = allUserIds.filter(
      userId => !selectedPlayers.includes(userId),
    );
  ...
  const isValidSelection =
    selectedPlayers.length > 0 && selectedPlayers.length % 12 === 0;
                          ...
                          <input
                            ...
                            value={
                              prepareAnnouncementData()
                                ? JSON.stringify(prepareAnnouncementData())
                                : ''
                            }
                          />
                          <button
              ...
              sortingData.ungroupedPlayers.length === 0 && (
                <div className='bg-green-900 bg-opacity-20 border border-green-600 rounded-lg p-4'>
                  <p className='text-green-200'>
                    ✅ Successfully sorted all {selectedPlayers.length} players
