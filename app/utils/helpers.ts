import { z } from 'zod';
import { getWeekNflGames } from '~/models/nflgame.server';

const envSchema = z.object({
  AWS_BUCKET: z.string().min(1),
  AWS_REGION: z.string().min(1),
  AWS_ENDPOINT: z.string().min(1),
  AWS_ACCESS_KEY_ID: z.string().min(1),
  AWS_SECRET_ACCESS_KEY: z.string().min(1),
  ADMIRAL_ROLE_ID: z.optional(z.string().min(1)),
  ADMIRAL_WAIVER_CHANNEL_ID: z.optional(z.string().min(1)),
  API_KEY: z.string().min(1),
  CHAMPIONS_ROLE_ID: z.optional(z.string().min(1)),
  CHAMPIONS_WAIVER_CHANNEL_ID: z.optional(z.string().min(1)),
  COOKIE_DOMAIN: z.optional(z.string().min(1)),
  DATABASE_URL: z.string().min(1),
  DEV_GUILD_ID: z.string().min(1),
  DISCORD_BOT_TOKEN: z.string().min(1),
  DISCORD_CLIENT_ID: z.string().min(1),
  DISCORD_SECRET: z.string().min(1),
  DRAGON_ROLE_ID: z.optional(z.string().min(1)),
  DRAGON_WAIVER_CHANNEL_ID: z.optional(z.string().min(1)),
  FORCE_ADMIN: z.enum(['off', 'on']),
  GALAXY_ROLE_ID: z.optional(z.string().min(1)),
  GALAXY_WAIVER_CHANNEL_ID: z.optional(z.string().min(1)),
  MONARCH_ROLE_ID: z.optional(z.string().min(1)),
  MONARCH_WAIVER_CHANNEL_ID: z.optional(z.string().min(1)),
  NODE_ENV: z.optional(z.string().min(1)),
  OMNI_CHANNEL_ID: z.optional(z.string().min(1)),
  OMNI_ROLE_ID: z.optional(z.string().min(1)),
  LEAGUE_ANNOUNCEMENT_CHANNEL_ID: z.optional(z.string().min(1)),
  SESSION_SECRET: z.string().min(1),
  WEBSITE_URL: z.string().min(1),
  SPREADS_API_KEY: z.string().min(1),
  PLAYWRIGHT_DISCORD_USERNAME: z.optional(z.string().min(1)),
  PLAYWRIGHT_DISCORD_PASSWORD: z.optional(z.string().min(1)),
  FFDISCORDADMIN_SLEEPER_ID: z.string().min(1),
});

const shuffleArray = <T>(array: T[]): T[] => {
  const arrayCopy = [...array];
  for (let i = arrayCopy.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [arrayCopy[i], arrayCopy[j]] = [arrayCopy[j], arrayCopy[i]];
  }
  return arrayCopy;
};

/**
 * Returns true if any NFL game for the given year/week is not complete.
 */
export async function areAllNflGamesComplete(
  year: number,
  week: number,
): Promise<boolean> {
  const nflGames = await getWeekNflGames(year, week);
  return nflGames.every(game => game.status === 'complete');
}

export { envSchema, shuffleArray };
